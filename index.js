 ...
 */
export function cleanObject(dirtyObject, allowMutations = false) {
 ...
 */
export function numStringToArr(str) {
 ...
 */
export function numArrToString(array) {
    if (array === undefined)
        return '';
    return array.join(', ');
}
 ...
 ** [1, 2, 3]
 */
export function customJsonStringify(object, properties, spaces = 1) {
 ...
 */
export async function persistentFetch(url, numberOfTries, data = undefined, r = 0) {
 ...
 * @returns Stored data
 */
export async function simpleIDB(name, key, payload) {
...
export async function fetchBungieManifest(locations, language = 'en') {
    ...
    for (let i = 0; i < locations.length; i++) {
        ...
        const fixedLocation = `Destiny${_.upperFirst(location)}Definition`;
